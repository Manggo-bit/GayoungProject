...
################################################################################
# Day 2: 로직 구성 및 문서화 집중                                                #
################################################################################

# ✅ 현재까지 문서화된 모든 로직은 다음 작업에 반영될 코드의 근간입니다.
# ==============================================================================
# 1. 다마고치 핵심 로직 최종 반영 및 개발일지 기록
# ==============================================================================

# 작업 내용: Gemini Guardian 프로젝트의 핵심인 다마고치 로직을 Development Log에 최종 반영하고,
#            이를 바탕으로 CRUD 데이터 모델을 설계할 준비를 합니다.
# ------------------------------------------------------------------------------
# 1-1. 생명 주기 (Life Cycle)
# ------------------------------------------------------------------------------
# 로직: 알(Egg) -> 해치(Hatch) -> 해츨링(Hatchling) -> 성체(Mature Pet)로 성장.
# - 앞으로 이 4단계 성장 로직을 Custom Hook에 구현할 예정입니다.
# ------------------------------------------------------------------------------
# 1-2. 상태 변화 (State) 및 성장 로직
# ------------------------------------------------------------------------------
# 로직: 퀴즈 정답 시 경험치(EXP) 획득 및 레벨(Level) 증가.
# - 성장 목표: 1문제, 3문제, 6문제, 11문제 누적으로 다음 단계 달성.
# - 상태(Status) 변동: 윤리적 딜레마 문제 정답률에 따라 **윤리적 상태**가 변동됩니다.
# ------------------------------------------------------------------------------
# 1-3. 최종 성장 분기 및 컨텐츠 (AI 활용 극대화)
# ------------------------------------------------------------------------------
# 로직: 사용자의 **학습 유형**에 따라 최종 펫의 종류와 특성 (예: 방어형, 윤리적 수호자)이 결정됩니다.
# - 앞으로 Gemini CLI를 활용하여 최종 펫 정보를 **JSON 포맷**으로 생성할 예정입니다.
# ------------------------------------------------------------------------------
# 1-4. 알림 (Alerts) 및 상호작용
# ------------------------------------------------------------------------------
# 로직: 펫의 '보안 상태' 또는 '활력'을 표시하며, 일정 시간마다 퀴즈를 풀도록 알림을 유도합니다.
# ------------------------------------------------------------------------------
# ⭐ 다음 단계는 Gemini CLI를 사용하여 이 로직 전체를 구현할 코드 요청입니다.
################################################################################